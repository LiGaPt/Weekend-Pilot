...
Generic recovery replay selectors are now also available for engineering verification: `--case-id <case_id>` runs one recovery-capable case, and `--suite-id recovery_focused` runs the registered 8-case recovery suite. Those additive selectors are not a new reviewer UI workflow and do not replace the canonical family default alias used by the current review package.
