...
- **V2 integrity gate**：运行 `python scripts/run_benchmark_v2_integrity_gate.py`，引用 `var/formal-benchmarks/latest-v2_integrity_gate-run-report.json`。
- **V2 repeated-run stability**：运行 `python scripts/run_benchmark_stability_passk.py --suite v2_integrity --runs 4`，引用 `var/formal-benchmarks/stability/latest-v2_integrity-passk-v0-report.json`。
...
- **评审目标**：评委可以据此分别核对 blocking 发布门槛、V1.5 多样性覆盖、V2 完整性门槛、重复运行稳定性、全量注册用例覆盖，以及标准失败恢复链条的闭环证据。
