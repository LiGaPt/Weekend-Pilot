...
当前提交口径已经收敛为 `V2 Integrity Edition`。`V1.5 baseline` 只作为已完成基线背景保留；当前正式评审重点是 benchmark 完整性、memory governance、observability 与 recovery 可审计性，而不是真实地图 provider 的深度集成。
...
- AMAP：可选只读提供方，目前覆盖部分读工具，默认测试不会调用真实服务；它在当前提交中只作为 API-only read-only preview，不进入 customer UI 主链，也不参与正式 benchmark。
...
- 多场景 Mock World 用例，覆盖亲子、朋友、单人、情侣、雨天、预算和老人同行等本地模拟 profile。
- 路线不可用、票务售罄、餐厅不可用、预算/排队约束等失败注入与组合失败用例。
...
- 更深入的真实地图提供方集成，例如 AMAP 或 Baidu MCP；这仍是后续方向，不是当前 `V2 Integrity Edition` 的主交付依赖。
