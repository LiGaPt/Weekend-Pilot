...
RECOVERY_LEVEL_COUNTS = {"L2": 1, "L5": 7}
RECOVERY_TOOL_PROFILE_COUNTS = {"mock_world": 8}
RECOVERY_WORLD_PROFILE_COUNTS = {
    "budget_lite": 1,
    "elder_afternoon": 1,
    "family_afternoon": 4,
    "friends_gathering": 1,
...
V2_INTEGRITY_LEVEL_COUNTS = {"L2": 6, "L3": 7, "L5": 7}
V2_INTEGRITY_TOOL_PROFILE_COUNTS = {"mock_world": 20}
...
V2_INTEGRITY_V2_LEVEL_COUNTS = {"L2": 5, "L3": 6, "L4": 1, "L5": 8}
...
V2_INTEGRITY_V2_STABILITY_REQUIRED_COUNTS = {"false": 6, "true": 14}
...
def test_load_benchmark_suite_returns_expected_named_membership() -> None:
    ...
    assert [case.case_id for case in load_benchmark_suite("v2_integrity")] == V2_INTEGRITY_CASE_IDS
...
def test_list_benchmark_suite_ids_for_case_returns_expected_membership() -> None:
    ...
    assert benchmark_suites.list_benchmark_suite_ids_for_case("friends_route_and_dining_unavailable_v1") == [
        "recovery_focused",
        "v2_integrity",
        "all_registered",
    ]
    ...
    assert benchmark_suites.list_benchmark_suite_ids_for_case("elder_ticket_sold_out_and_route_unavailable_v1") == [
        "recovery_focused",
        "v2_integrity",
        "all_registered",
    ]
...
def test_list_benchmark_suites_returns_descriptions_in_deterministic_order() -> None:
    ...
    _assert_suite_description(
        ...
        world_profile_counts=V2_INTEGRITY_WORLD_PROFILE_COUNTS,
        failure_mode_counts=V2_INTEGRITY_FAILURE_MODE_COUNTS,
        tag_counts=V2_INTEGRITY_TAG_COUNTS,
    )
    assert suite_map["v2_integrity"].v2_taxonomy_summary is not None
    assert suite_map["v2_integrity"].v2_taxonomy_summary.level_counts == V2_INTEGRITY_V2_LEVEL_COUNTS
    assert (
        suite_map["v2_integrity"].v2_taxonomy_summary.memory_mode_counts
        == V2_INTEGRITY_V2_MEMORY_MODE_COUNTS
    )
    assert (
        suite_map["v2_integrity"].v2_taxonomy_summary.conversation_mode_counts
        == V2_INTEGRITY_V2_CONVERSATION_MODE_COUNTS
    )
    assert (
...
def test_memory_governance_and_integrity_suites_expand_for_v2_memory_cases() -> None:
    assert [case.case_id for case in load_benchmark_suite("memory_governance")] == [
        "family_memory_override_v1",
        "family_memory_advisory_fill_v1",
        "family_memory_expired_advisory_v1",
        "family_memory_disabled_ignored_v1",
    ...
    assert [case.case_id for case in load_benchmark_suite("v2_integrity")] == [
        "family_memory_override_v1",
        "family_route_failure_v1",
        "family_route_and_dining_unavailable_v1",
        "friends_route_and_dining_unavailable_v1",
    ...
    assert len(load_benchmark_suite("all_registered")) == 30
...
def test_list_benchmark_suites_rejects_duplicate_case_ids(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ...
        "_SUITE_DEFINITIONS",
        {
            ...
            "v2_integrity": {
                "title": "V2 integrity suite",
                "description": "Integrity cases",
                "case_ids": V2_INTEGRITY_CASE_IDS,
            },
            "all_registered": {
