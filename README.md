...
Every successful public `DemoRunSummary` now also includes an additive `progress` object with `schema_version`, `current_stage`, `current_label`, and ordered `stage_history`. The backend derives that snapshot from persisted workflow node history, ordered tool events, and demo continuation history only, so `GET /demo/runs/{run_id}` can reconstruct the same public-safe progress view after the run completes. Live mid-request transport is still out of scope here: this task does not add async start, polling, SSE, or WebSockets.
