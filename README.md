...
Every successful public `DemoRunSummary` now also includes an additive `progress` object with `schema_version`, `current_stage`, `current_label`, ordered `stage_history`, and customer-safe `steps`. Each `progress.steps[]` item contains `stage`, `label`, `status`, and `summary`, so the customer page can render one compact progress stepper with the current step highlighted and completed steps folded behind a disclosure. For the default Mock World family path, those summaries now include concrete search milestones such as `已找到 5 个活动` and `已找到 5 个餐厅` when the persisted search evidence supports them. The backend still derives the whole snapshot from persisted workflow node history, ordered tool events, plan rows, and demo continuation history only, so `GET /demo/runs/{run_id}` can reconstruct the same public-safe progress view after the run completes. Live mid-request transport now exists for the initial start path through `POST /demo/runs/stream`; broader async follow-up routes, polling, and WebSockets remain out of scope.
Confirmed runs now also persist `plans[*].feedback.final_arrangement_message` as an additive customer-facing field under the existing feedback object. The customer result card promotes this copyable final arrangement message above the lower-level execution timeline while keeping the generic execution feedback fields and disclosure behavior unchanged.
...
Update: the customer page now starts the initial planning request through `POST /demo/runs/stream` instead of waiting on synchronous `POST /demo/runs`. Because this stream route is a `POST`, the frontend consumes it with `fetch` plus `ReadableStream` SSE parsing rather than browser `EventSource`. The stream accepts the same request body as `POST /demo/runs`, including optional `mock_world_profile`, emits `progress` events with the public-safe `DemoProgressSummary`, ends with one `summary` event carrying the full `DemoRunSummary`, and can emit one `error` event with a public-safe message. If the stream transport fails before any valid live `progress` event is received, the frontend now retries the same start request once through synchronous `POST /demo/runs`, reusing the identical request body so the selected scenario profile is preserved. For the default Mock World happy path, the stream now emits ordered search milestones inside the existing `progress` event type: first `searching_activities`, then `searching_dining`, before later stages such as `checking_availability`. Clarify, replan, confirm, decline, and readback remain synchronous in this task. This task still does not add polling, WebSockets, reconnect cursors, background async execution, or streamed follow-up routes.
...
The same customer page now also exposes six fixed Mock World scenario chips in start mode only: `亲子`, `朋友`, `单人`, `情侣`, `雨天`, and `预算`. Clicking a chip only fills the composer and binds the matching `mock_world_profile` to the next start request; it does not auto-submit and it does not appear in clarification or replan mode. The customer page still does not expose a first-screen AMap selector.
