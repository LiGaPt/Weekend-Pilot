...
| 技术支撑 | `5174` 内部评审页提供 `Benchmark Summary`、`System Integrity Summary`、`Run Summary`、`Selected Plan Review`、`Trace Summary`、`Benchmark Artifacts` 与 `Recovery Visualization`，其中 `Benchmark Summary` 会展示 suite 级 timing percentile 与 stage 分布 |
...
- 内部评审页已经固定为 `http://127.0.0.1:5174/`，可展示 `Benchmark Summary`、`System Integrity Summary`、`Run Summary`、`Selected Plan Review`、`Trace Summary`、`Benchmark Artifacts`、`Recovery Visualization`，并在 `Benchmark Summary` 中展示 suite 级 `p50 / p95 / p99 / max` 与 stage timing percentile
